...
export class VolunteersPageService {
  constructor(
    @InjectRepository(VolunteersPage) private readonly pageRepo: Repository<VolunteersPage>,
  ...
  async get(): Promise<VolunteersPage> {
    ...
    if (!page) throw new NotFoundException('No hay contenido para VolunteersPage.');
    return page;
  }

  async upsert(payload: UpsertVolunteersPageDto): Promise<VolunteersPage> {
    return await this.ds.transaction(async (trx) => {
      // buscar la única fila existente (si hay)
      let page = await trx.getRepository(VolunteersPage).findOne({ where: {} });

      if (!page) {
        // crear página
        page = trx.getRepository(VolunteersPage).create({
          headerTitle: payload.headerTitle,
          headerDescription: payload.headerDescription,
        });
        page = await trx.getRepository(VolunteersPage).save(page);
      } else {
        // actualizar encabezado
        page.headerTitle = payload.headerTitle;
        page.headerDescription = payload.headerDescription;
        await trx.getRepository(VolunteersPage).save(page);
      ...
      const reloaded = await trx.getRepository(VolunteersPage).findOneOrFail({
        where: { id: page.id },
        relations: ['benefits', 'requirements'],
        order: {
          benefits: { order: 'ASC' },
  ...
  async seedIfEmpty(): Promise<void> {
    ...
    await this.upsert({
      headerTitle: '¿Por qué ser voluntario en la Cámara de Ganaderos?',
      headerDescription:
        'Ser voluntario te permite impulsar el crecimiento del sector ganadero local, fortalecer el desarrollo sostenible de la comunidad y formar parte activa de iniciativas que promueven el bienestar del agro y el medio ambiente.',
      benefits: [
